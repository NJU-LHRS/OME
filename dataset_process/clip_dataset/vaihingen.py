...
def parse_args():
    ...
    parser.add_argument('--dataset_path', default=r'ISPRS semantic label data\Vaihingen',
                        help='vaihingen folder path')
...
def clip_big_image(image_path, clip_save_dir, to_label=False):
    ...
    for i, box in enumerate(boxes):
        ...
        mmcv.imwrite(
            ...
            osp.join(clip_save_dir, f'{area_idx}_{str(i).zfill(3)}_{start_x}_{start_y}_{end_x}_{end_y}.png'))
...
def main():
    ...
    if args.out_dir is None:
        if args.noboundary:
            out_dir = osp.join(r'../../data', f'vaihingen_IRRG_noB_{args.clip_size}_{args.stride_size}')
        else:
            out_dir = osp.join(r'../../data', f'vaihingen_IRRG_wiB_{args.clip_size}_{args.stride_size}')
    else:
    ...
    if args.noboundary:
        zipp_list = [os.path.join(dataset_path,f'{name}.zip') for name in ['ISPRS_semantic_labeling_Vaihingen_top',
    ...
    with tempfile.TemporaryDirectory(dir=args.tmp_dir) as tmp_dir:
        for zipp in zipp_list:
            ...
            if 'ISPRS_semantic_labeling_Vaihingen_top' in zipp:
                ...
                if not len(src_path_list):
                    ...
                    src_path_list = glob.glob(os.path.join(sub_tmp_dir, 'top', '*.tif'))
            if 'ISPRS_semantic_labeling_Vaihingen_ground_truth' in zipp:  # noqa
                src_path_list = glob.glob(os.path.join(tmp_dir, '*.tif'))
                if not len(src_path_list):
                    sub_tmp_dir = os.path.join(tmp_dir, os.listdir(tmp_dir)[0])
            ...
            for i, src_path in enumerate(src_path_list):
                ...
                data_type = 'train' if area_idx in splits['train'] else 'test'
                if 'ground_truth' in src_path:
                    dst_dir = osp.join(out_dir, 'ann_dir', data_type)
                    clip_big_image(src_path, dst_dir, to_label=True)
                    dst_dir_color = osp.join(out_dir, 'ann_dir', data_type+'_color')
                    clip_big_image(src_path, dst_dir_color, to_label=False)
                else:
